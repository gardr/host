...
proto.isViewable = function() {
    return this.options.container &&
        this.options.container.parentNode &&
        document.body.contains(this.options.container);
};

proto.set = function(input) {
    this.lastState = this.state;
    var newState = (typeof input == 'number') ? input : State[input];
    if (typeof newState === 'undefined') {
        throw new TypeError('State needs a number');
    }
    this.state = newState;
};
...
proto.updateData = function(options) {
    for (var key in options) {
        if (key === 'data' && this.options.data) {
            this.options.data = extend(this.options.data, options.data);
        } else {
