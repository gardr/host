...
describe('Manager', function () {
    ...
    describe('config', function(){
        ...
        describe('queuing config with multiple settings', function () {
            var name = helpers.getRandomName();
            ...
            var config = manager.get(name);
            var item1 = manager.get(name + '_0');
            var item2 = manager.get(name + '_1');

            it('should create new with count index', function () {
                expect(config).toBeDefined();
                expect(item1).toBeDefined();
                expect(item2).toBeDefined();
            });

            it('should inherit options, but not multiple property', function () {
                expect(config.multiple).toBe(true);
                expect(item1.multiple).toBeUndefined();
                expect(item2.multiple).toBeUndefined();

                // should inherit data/config
                expect(item1.url).toEqual(scriptUrl);
                expect(item1.val).toEqual(val);
                expect(item2.val).toEqual(val);
            });

            it('calling render on parent config should render all', function () {
                var done = false;

                manager.render(name, function (err, configItem) {
                    expect(err).toBeUndefined();
                    expect(configItem).toBeDefined();
                    expect(configItem).toEqual(manager.get(name));
                    done = true;
                });

                manager.resolve(name + '_0');
                manager.resolve(name + '_1');

                expect(manager.get(name + '_0').isActive()).toEqual(true);
                expect(manager.get(name + '_0').isResolved()).toEqual(true);
